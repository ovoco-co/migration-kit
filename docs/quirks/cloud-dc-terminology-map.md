...
| Workflow post-functions | Automation rules only; no Groovy | Assets post-functions (create/update/set attribute), Groovy scripts, conditions, validators | Complex workflow logic (e.g., create Asset on transition, set attributes from issue fields) must be rebuilt using Cloud Automation rules, which may not cover all DC post-function capabilities. |
...
| Groovy-based integrations | Not available; use Forge, Connect, or REST API scripts | Full Groovy scripting via Assets workflow functions and ScriptRunner | All Groovy-based automations, custom fields, and workflow logic must be redesigned for Cloud. |
